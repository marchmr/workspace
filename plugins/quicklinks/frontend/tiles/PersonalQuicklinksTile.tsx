...
/* ════════════════════════════════════════════
   Personal Quicklinks Tile (Read-Only)
   viewMode: 'list' = vertikal | 'columns' = horizontal nebeneinander
   ════════════════════════════════════════════ */

export default function PersonalQuicklinksTile({ viewMode = 'list' }: { viewMode?: string }) {
    ...
    const load = useCallback(async () => {
        try {
            const res = await apiFetch('/api/plugins/quicklinks/personal');
            if (res.ok) {
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            ...
            {links.length === 0 ? (
                <div className="text-muted"
                    onClick={() => navigate('/quicklinks')}
                    style={{ textAlign: 'center', padding: 'var(--space-md)', fontSize: 'var(--font-size-sm)', cursor: 'pointer' }}>
                    Noch keine Links — klicke um welche zu erstellen
                </div>
